...
function AuthForm({ mode, onAuthed }: { mode: 'setup' | 'login'; onAuthed: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault()
    console.log('[AuthForm] Submit clicked', { email, password, emailError, passwordError });
    if (emailError || passwordError) {
      console.log('[AuthForm] Validation failed. Aborting submit.');
      setAttempted(true)
      return
    }
    setBusy(true)
    setError('')
    console.log('[AuthForm] Sending API request...');
    try {
      ...
      console.log('[AuthForm] API request succeeded, received token');
      setToken(res.token)
      onAuthed()
    } catch (err) {
      console.error('[AuthForm] API request failed:', err);
  ...
  return (
    <Centered>
      <div className="mb-6 flex items-center gap-2.5">
        ...
        <span className="font-extrabold tracking-wider text-base bg-gradient-to-r from-cyan-400 via-teal-300 to-cyan-500 bg-clip-text text-transparent filter drop-shadow-[0_0_8px_rgba(6,182,212,0.5)]">
          TrungSaaS
        </span>
      </div>
      <div className="glass-mirror shadow-3d-cyan hover-3d-cyan rounded-3xl p-6 border border-cyan-500/20 dark:border-cyan-500/30 transition-all duration-300">
