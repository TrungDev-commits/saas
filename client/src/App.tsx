...
import DocsQAPage from '@/pages/DocsQAPage'
import SkillsPage from '@/pages/SkillsPage'

const navItems = [
  { to: '/models', labelKey: 'nav.models' },
  { to: '/playground', labelKey: 'nav.playground' },
  { to: '/docs-qa', label: 'Docs Q&A' },
  { to: '/skills', label: 'Skills/Experts' },
...
function Brand() {
  return (
    <Link to="/" className="flex items-center gap-2.5 transition-all hover:opacity-95 hover:scale-[1.02] active:scale-95 group">
      <span className="relative flex size-3">
        <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-cyan-400 opacity-75"></span>
        <span className="relative inline-flex rounded-full size-3 bg-cyan-400 shadow-[0_0_8px_#00f0ff]"></span>
      </span>
      <span className="font-extrabold tracking-wider text-base bg-gradient-to-r from-cyan-400 via-teal-300 to-cyan-500 bg-clip-text text-transparent group-hover:from-cyan-300 group-hover:to-cyan-400 transition-all filter drop-shadow-[0_0_8px_rgba(6,182,212,0.5)]">
        TrungSaaS
      </span>
    </Link>
  )
...
function Navbar() {
  ...
  return (
    <header
      ...
      className={`sticky top-0 z-40 border-b border-cyan-500/10 backdrop-blur-xl ${isDesktopApp ? 'bg-background/40' : 'bg-background/65'} shadow-[0_4px_30px_rgba(0,240,255,0.02)]`}
          ...
          {navItems.map((item) =>
            item.to === '/models' ? (
              ...
              <div key={item.to} className="flex items-center gap-0.5">
                <NavItem to={item.to}>{item.labelKey ? t(item.labelKey) : item.label}</NavItem>
                <DropdownMenu>
                  <DropdownMenuTrigger
          ...
          <div className={`min-h-screen ${isDesktopApp ? 'desktop-backdrop' : 'bg-background'}`}>
            ...
            <main className="max-w-6xl mx-auto px-6 py-8">
              ...
              <Routes>
                ...
                <Route path="/docs-qa" element={<DocsQAPage />} />
