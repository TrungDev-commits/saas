...
function fusionRouteLabel(p: { platform: string; model: string }): string {
  return p.model.startsWith(`${p.platform}/`)
    ? p.model
    : `${p.platform}/${p.model}`;
}
  ...
  return (
    <div className="w-full text-[10px] leading-snug text-muted-foreground/80">
      ...
      >
        ...
        />
        {t("playground.fusionTrace", { count: panel.length })}
        {streaming ? " …" : ""}
      </button>
      {open && (
        <div className="mt-1 space-y-2 border-l border-border/60 pl-2.5">
          ...
          {judge && (
            <div className="pt-1.5 border-t border-border/60">
              <span className="font-mono font-medium">
                {fusionRouteLabel(judge)}
              </span>
              <span className="ml-1.5 opacity-70">
                {t("playground.fusionJudgeSynth")}
              </span>
            </div>
          )}
        </div>
...
export default function PlaygroundPage() {
      ...
      for (const line of lines) {
        ...
        if (obj._fusion) {
          if (obj._fusion.event === "panel") {
            panel.push({
              platform: obj._fusion.platform,
              model: obj._fusion.model,
              status: obj._fusion.status,
              content: obj._fusion.content,
              error: obj._fusion.error,
            });
  ...
  const pickerOptions = [
    ...
    {
      value: "fusion",
      label: t("playground.fusionModel"),
      sub: "",
      isNew: true,
      platforms: [] as string[],
    },
      ...
      <div className="flex-1 flex flex-col rounded-3xl border bg-card overflow-hidden min-h-0">
          ...
          {messages.length === 0 ? (
            ...
            <>
              {messages.map((msg, i) => {
                ...
                const okPanel =
                  fusionPanel?.filter((p) => p.status !== "failed") ?? [];
                                ...
                                <>
                                  ...
                                  {msg.meta.fusionJudge && (
                                    <span>
                                      ...
                                      <span className="font-mono">
                                        {fusionRouteLabel(msg.meta.fusionJudge)}
                                      </span>
                                    </span>
                                  )}
                                  ...
                                  {msg.meta.fallbackAttempts != null &&
                                    msg.meta.fallbackAttempts > 0 && (
                                      <span>
                                        ...
                                        {msg.meta.fallbackAttempts > 1
                                          ? t("playground.fallbacks")
                                          : t("playground.fallback")}
                                      </span>
                                    )}
                                </>
                              )}
              ...
              {loading &&
                !messages[messages.length - 1]?.meta?.fusionStreaming && (
        ...
        <div className="border-t bg-background/50 p-3">
          <div className="mb-2">
            ...
            >
              <ChevronRight
                className={`size-3.5 transition-transform ${systemPromptOpen ? "rotate-90" : ""}`}
              />
