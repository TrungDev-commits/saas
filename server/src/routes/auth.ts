...
authRouter.get('/status', (req: Request, res: Response) => {
  console.log('[AuthRouter] GET /status - Headers:', req.headers);
...
authRouter.post('/setup', (req: Request, res: Response) => {
  console.log('[AuthRouter] POST /setup - Body:', req.body, 'Remote IP:', req.socket.remoteAddress);
  if (userCount() > 0) {
    clearSetupCode();
  ...
  const user = createUser(parsed.data.email, parsed.data.password, 'admin');
  clearSetupCode(); // one-time: the dashboard is now claimed
  const token = createSession(user.userId);
  res.status(201).json({ token, email: user.email, role: 'admin' }); // First user setup is admin
});

authRouter.post('/login', (req: Request, res: Response) => {
  console.log('[AuthRouter] POST /login - Body:', req.body);
  ...
  let role = 'user';
  try {
    const userRow = getDb().prepare('SELECT role FROM users WHERE id = ?').get(user.userId) as { role: string } | undefined;
    role = userRow?.role ?? 'user';
  } catch {}
